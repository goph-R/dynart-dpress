...
**An attachment is a file somebody attached; an image in the body is a reference in the text.** Two different things, kept in two different places. The field's *Insert from library* button writes a reference and attaches **nothing** - and **what it writes is decided by the category** (`Dpress.mediaMarkdown()`): an image is `![alt](media#12)`, a video is `{{ video('media#13') }}`, an audio file is `{{ audio('media#5') }}`, and anything else is a link, because a PDF is a download. A video used to be written as an image, which renders an `<img>` pointing at an mp4; "Add attachment" attaches and writes nothing. A file can be both, either or neither, and **nothing infers any of it from the text** — 0.15.0 reconciled attachments against the markdown on save and it was withdrawn in 0.16.0 because it fights the author, and 0.24.0 removed `ContentAttachment.hidden`, which only existed because inserting a picture used to attach it. Removing a file from the text is a separate act from detaching it, and both are the author's.
...
**Featured posts are a tag, and the strip is the front page's furniture.** `featured_tag` names it (`featured` by default, empty for none), `HomeController` hands the theme `$featured_posts` — at most five, newest first — and passes their ids to `content_list` as **`exclude_ids`** so the same post is not pinned at the top *and* repeated four rows down. A tag rather than a `featured` column because an author already knows how to tag a post and un-featuring is removing one: no new screen, no migration. **`$featured_posts`, never `$featured`** — the single-post templates have used `$featured` for the post's own picture since there were templates. **The tag itself is hidden from visitors**, in the cloud and in the sitemap, through `tagCloud(['exclude_slug' => ...])` - left out of the rows rather than filtered after them, so it is out of the cloud's weight scaling too, which it would otherwise set the top of on its own. The exclusion is **opt-in and not a default**, because `taxonomy:list` calls the same query and hiding a tag from the listing that would tell you it exists is how it gets lost. `TaxonomyService::featuredTagSlug()` is the one place the setting is read.

In `applyContentFilters()` the exclusions are one `<>` condition each rather than one `not in`: `Query::nextParamName()` only sees a name once it is **bound**, so asking for several before adding anything hands back the same name every time.
...
**`PageContext` is what the page being rendered is *about*.** A block renderer is handed `(Block, array $settings)` and nothing else, which is right for a tag cloud and useless for a comments block that has to name its thread. A service rather than an argument threaded through `Places::render()`, because a shortcode is not rendered through a place and wants the same answer. **Empty is the normal case** - the front page, an archive and every admin screen are about no content at all - so a block that needs one renders nothing there rather than guessing. Set in `AbstractController::renderContent()`, the one place a post or a page becomes HTML.

**`after_content` is the place under a post and under a page**, outside the `</article>` because what goes there is about what was just read rather than part of it. One place and not two, so "comments on pages as well" is a question about where the block is put. **A theme declaring its own `places[]` replaces the built-in list rather than adding to it**, so a theme wanting it has to name it.

**`/feed` and `/sitemap.xml` are documents, not pages.** Both send bytes and call `finish()` like `MediaController`, because neither is HTML and neither must reach the layout, the shortcodes or `PageAssets`. Both build their document in a service - `Feed`, `Sitemap` - so the controller only sets headers, and both take the **rendered** columns rather than re-rendering: `lead_html` and `body_html` are already absolute because `content:rerender` writes URLs against `app.base_url`, so there is no second markdown pipeline to keep in step with the first.

The feed's `<link rel="alternate">` is registered into `PageAssets` with `</head>` as its needle, which is not a trick - it is exactly the question being asked, and it keeps a fragment render from building the service to find out it is not a page. **A trailing slash is the app skeleton's job**: WordPress wrote `/feed/`, and `public/.htaccess` redirects any trailing slash before PHP starts, which is one rule for every URL rather than an alias route for this one.

Two traps, both with tests that fail without the fix. **A post containing `]]>`** ends a CDATA section early and the feed stops being XML from there down, so a reader drops the whole document rather than that item - it is split across two sections. And **a draft ancestor still shapes a published child's URL**, since `ContentService::path()` never asks about status, so the sitemap fetches every page and lists only the published ones. The sitemap claims no `changefreq` and no `priority` - Google ignores both, and they are the fields that make a hand-written sitemap look authoritative while saying nothing.

**Settings are a registry** (`SettingFields`), not the `const FIELDS` they were until 0.62.0. A plugin could add a field to the settings form through `form.admin_settings:created`, watch it render, and watch it silently not be saved, because the loop that writes them iterated a constant it had no way into - an extension point that *appears* to work, which is worse than a missing one. `add($name, $type, $field)` takes the form field too, so a plugin needs one call; core's twelve pass none, because theirs need lists only the controller can fetch.
...
**A plugin reaches a visitor through `PageAssets`.** A registry for the `<head>` of a page somebody asked for, and the front end's half of `FormWidgets` and `Blocks`. Every entry carries a **needle** - a plain substring the finished page has to contain - so an icon font is on the pages with an icon on them and nowhere else, and a site with three plugins enabled and none of them on this page loads nothing for any of them. It runs over the finished page in `AbstractController::render()`, for the reason the shortcodes do: what is *on* a page is a fact about the output, not about the controller. The syntax highlighter is registered into it rather than special-cased, which is the point - a mechanism the core does not eat is a mechanism nobody tests. A plugin declares files with `pageAssets()` (`file => needle`), served from `/assets/plugin/<plugin>/<file>` with the **plugin's** version as the buster, and they are registered as a **closure**: the loader runs in the CLI too, where there is no router to build a URL with, and a plugin doing it eagerly would be recorded as failed by `dpress upgrade`.
...
**The markdown field is a textarea, deliberately not an editor.** A markdown field whose value is anything other than what the author typed eventually rewrites somebody's document on save, and the content model is "the markdown is the truth". It is **coloured** since 0.67.0 and that changes nothing about it: `assets/markdown-highlight.js` puts a `<pre>` behind the textarea holding the same characters in colour and turns the field's own text transparent over the top, so the value, the selection, the undo stack and the no-JavaScript fallback are all untouched and nothing in the highlighter can write back. **The grammar is ours** rather than a library's - an off-the-shelf mode knows `**bold**` and nothing about `media#12`, `{{ shortcode() }}` or `> [!WARNING]`, and those are the marks worth confirming while typing. The `---` rule is `MarkdownRenderer::separatorLines()` transcribed and a test greps for the constants it copied, because a highlighter that disagrees with the splitter about where a post breaks is worse than none. **No metric is written twice**: everything that decides where a character lands is read off the textarea onto the backdrop, so `textarea.markdown-editor` in `admin.css` stays the one place the field's font and padding are chosen, and the scrollbar's gutter is added to the backdrop's padding or the two wrap at different columns. Two palettes, Dracula in the dark scheme and the same roles at 4.5:1 on white in the light one. **The toolbar's formatting buttons are gone** (0.67.0): they wrote marks shorter to type than to reach for. What is left is the two things a keyboard cannot do - the library insert button, because `![alt](media#12)` needs an id the author does not have, and the **emoji picker** (0.68.0), because nobody remembers a code point. An emoji is **text**, so `assets/emoji.js` is a curated list and a dialog and touches no server: what goes in the document is the character, which `SchemaService::CHARSET` (`utf8mb4`) has always been able to store. The list is split on a **space**, because splitting by character tears a zero-width-joined family into people. **The keywords are `assets/emoji-words.js`, keyed by the character** - a list beside the list goes out of step on the first insertion and answers the wrong face for everything after it, where a map cannot drift; a test asserts every emoji has an entry and none is left over. It is a separate file because it is data where the picker is behaviour, and it is optional: without it the search finds nothing and the picker still works. `search()` returns **groups**, not a flat list, which is what keeps the tab row and the sections from disagreeing about which groups matched - and a term matches the *start of a word* because `art` answering with `heart` reads as not understanding the question. **The picker shows one group while browsing and every matching group while searching**, and that is a performance decision rather than a design one: a colour emoji is an image the browser paints, and 907 of them in a scrolling box is enough work per frame to be felt on a laptop. It was tried as sections in one scroll, with the headings sticky and then not, and neither helped - **the number of glyphs on screen was the whole problem**, so a tab now shows at most 155. A search can afford every group because a search is a handful of hits. A field with neither button grows no bar at all.
