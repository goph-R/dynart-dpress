...
The panel writes immediately over `Dpress.send()` rather than on Save, so the form is never reloaded and there is one write model — the same as every other row action. It therefore needs a saved post, and on a new one it does not render at all rather than explaining a disabled button. The toolbar's image button is unaffected: a reference needs no id, so inserting a picture works before the first save.
